...
export type IconSize = "xxs" | "xs" | "s" | "n" | "m" | "l" | "xl" | "xxl";

const IconSizeValue: { [key: string]: number } = {
    "xxs": 12,
    "xs": 16,
    "s": 20,
    "n": 24,
...
export abstract class IconBase extends HTMLElement {
    ...
    protected static getFontSize(iconSize: IconSize | null): number {
        return iconSize ? (IconSizeValue[iconSize] ?? IconSizeValue["s"]) : IconSizeValue["s"];
    }

    protected abstract getFontFamily(): string;
    protected abstract getGlyph(): string | null;

    protected createInner(shadow: ShadowRoot) {
        const size: number = IconBase.getFontSize(this.getAttribute("size") as IconSize);
